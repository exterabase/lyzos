document.addEventListener('DOMContentLoaded', () => {
  ...
  async function updateDiscordStatus() {
    try {
      ...
      if (data.success) {
        ...
        username.textContent = discord.discord_user.global_name || discord.discord_user.username;
  ...
  function showFallback() {
    // Lanyard не видит юзера (он не на сервере discord.gg/lanyard) —
    // прячем битую аватарку вместо alt-текста и показываем статичный фолбэк.
    avatar.style.display = 'none';
    username.textContent = 'lyzos';
    statusDot.className = 'status-offline';
    statusText.textContent = 'Не отслеживается';
    activityInfo.classList.add('hidden');
    noActivity.classList.remove('hidden');
  }
